        ...
        if start_location:
            if ego_vehicle_route:
                if config.route_var_name is None:  # pylint: disable=no-else-return
                    print(f'BASIC SCENARIO: {self.name}: waiting for the ego car to appear in {start_location} to start the scenario')
                    ...
                    print(f'BASIC SCENARIO: {self.name}: waiting for {config.route_var_name} to be set to TRUE to start the scenario')
