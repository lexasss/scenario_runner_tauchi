...
from utils import add_carla_path
add_carla_path()
...
def start_scenario_runner(scenario_runner_instance):
    try:
        print("EXPERIMENT RUNNER: starting . . .")
    ...
    print("EXPERIMENT RUNNER: stopped, result:", result)
...
def start_recording(client, args, scenario_runner_instance):
    ...
    print("EXPERIMENT RUNNER: recording on file: %s" % recorder_file)


def stop_recording(client):
    global recorder_file
    print("EXPERIMENT RUNNER: stopping recording, file saved to %s" % recorder_file)
...
def scenario_runner_args(parser):
    ...
    parser.add_argument('--params', default='', help='Any custom parameters to pass to the scenario')
...
def main():
    ...
    if not args.route:
        print("EXPERIMENT RUNNER: please specify the route mode, other modes are not supported\n\n")
