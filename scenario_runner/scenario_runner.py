...
class ScenarioRunner(object):
    ...
    def _get_scenario_class_or_fail(self, scenario):
        ...
        print("SCENARIO RUNNER: Scenario '{}' not supported ... Exiting".format(scenario))
        sys.exit(-1)
        ...
        for i, _ in enumerate(self.ego_vehicles):
            if self.ego_vehicles[i]:
                if not self._args.waitForEgo and self.ego_vehicles[i] is not None and self.ego_vehicles[i].is_alive:
                    print("SCENARIO RUNNER: Destroying ego vehicle {}".format(self.ego_vehicles[i].id))
    ...
    def _analyze_scenario(self, config):
        ...
        if not self.manager.analyze_scenario(self._args.output, filename, junit_filename, json_filename):
            print("SCENARIO RUNNER: All scenario tests were passed successfully!")
        else:
            print("SCENARIO RUNNER: Not all scenario tests were successful")
            if not (self._args.output or filename or junit_filename):
                print("SCENARIO RUNNER: Please run with --output for further information")
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
                    ...
                    for ego_vehicle in ego_vehicles:
                        ...
                        for vehicle in vehicles:
                            if vehicle.attributes['role_name'] == ego_vehicle.rolename:
                                ...
                                break
                        if not ego_vehicle_found:
                            print("SCENARIO RUNNER: Not all ego vehicles ready. Waiting ... ")
                            time.sleep(1)
                            break
        ...
        if map_name not in (town, "OpenDriveMap"):
            print("SCENARIO RUNNER: The CARLA server uses the wrong map: {}".format(map_name))
            print("SCENARIO RUNNER: This scenario requires to use map: {}".format(town))
            return False
    ...
    def _load_and_run_scenario(self, config):
        ...
        if self._args.agent:
            ...
            try:
                ...
                traceback.print_exc()
                print("SCENARIO RUNNER: Could not setup required agent due to {}".format(e))
                self._cleanup()
                return False
        ...
        # Prepare scenario
        print("SCENARIO RUNNER: Preparing scenario " + config.name)
        try:
            ...
            print("SCENARIO RUNNER: The scenario cannot be loaded")
            traceback.print_exc()
            print(exception)
            self._cleanup()
            return False
    ...
    def _run_scenarios(self):
        ...
        if not scenario_configurations:
            print("SCENARIO RUNNER: Configuration for scenario {} cannot be found!".format(self._args.scenario))
            return result
    ...
    def _run_openscenario(self):
        ...
        if not os.path.isfile(self._args.openscenario):
            print("SCENARIO RUNNER: File does not exist")
            self._cleanup()
            return False
    ...
    def run(self):
        ...
        print("SCENARIO RUNNER: No more scenarios .... Exiting")
        return result
...
def main():
    ...
    if arguments.list:
        print("SCENARIO RUNNER: Currently the following scenarios are supported:")
    ...
    if not arguments.scenario and not arguments.openscenario and not arguments.route:
        print("SCENARIO RUNNER: Please specify either a scenario or use the route mode\n\n")
        parser.print_help(sys.stdout)
        return 1

    if arguments.route and (arguments.openscenario or arguments.scenario):
        print("SCENARIO RUNNER: The route mode cannot be used together with a scenario (incl. OpenSCENARIO)'\n\n")
        parser.print_help(sys.stdout)
        return 1

    if arguments.agent and (arguments.openscenario or arguments.scenario):
        print("SCENARIO RUNNER: Agents are currently only compatible with route scenarios'\n\n")
        parser.print_help(sys.stdout)
        return 1

    if arguments.openscenarioparams and not arguments.openscenario:
        print("SCENARIO RUNNER: Warnings: ignoring --openscenarioparams when --openscenario is not specified")
